...
qn_h = solveFor_qn_h(FBdry, antiBCs, V_q)
qn_h_inconsistent = solveFor_qn_h(FBdry_inconsistent, antiBCs, V_q)
qn_h_full = solveFor_qn_h(FBdry_full, interiorBCs, V_q)
...
plt.plot(np.linspace(0, 1), [qn_h(x_left, y) for y in np.linspace(0, 1)], ':', label="consistent left edge")
plt.plot(np.linspace(1, 2), [qn_h(x, y_top) for x in np.linspace(0, 1)], ':', label="consistent top edge")
plt.plot(np.linspace(2, 3), [qn_h(x_coupling, y) for y in np.linspace(1, 0)], '-x', label="consistent right edge")
plt.plot(np.linspace(3, 4), [qn_h(x, y_bottom) for x in np.linspace(1, 0)], ':', label="consistent bottom edge")
